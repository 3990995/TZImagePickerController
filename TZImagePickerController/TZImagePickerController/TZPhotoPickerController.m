...
#define Sort(a) [[a componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] componentsJoinedByString:@" "]
//#define LANGUAGE(key) NSLocalizedString(key, nil)
#define LANGUAGE(key) Sort(NSLocalizedString(key, nil))
...
@property (assign, nonatomic) BOOL canTake ;

@property (strong, nonatomic) UIImageView *markButton;
...
- (void)viewDidLoad {
    ...
    UIImageView *markButton = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"相册标记"]];
    ...
    markLabel.text = LANGUAGE(@"brandu_album_last_view_txt");
    UIFont *font;
    ...
    [markButton addSubview:markLabel];
    [markLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(markButton).offset(31.0);
        make.right.centerY.equalTo(markButton);
    }];
}
...
- (void)configBottomToolBar {
    ...
    [_doneButton addTarget:self action:@selector(doneButtonClickToClicked:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)viewDidLayoutSubviews {
    ...
    [self.view bringSubviewToFront:_markButton];
}
...
- (void)doneButtonClickToClicked:(id)sender {
    _doneButton.enabled = NO;
    [self doneButtonClick];
    [self performSelector:@selector(changeButtonStatus)withObject:nil afterDelay:1.0f];//防止重复点击
//    //点击按钮后先取消之前的操作，再进行需要进行的操作
//    [[self class] cancelPreviousPerformRequestsWithTarget:self selector:@selector(doneButtonClick) object:sender];
//    [self performSelector:@selector(changeButtonStatus) withObject:sender afterDelay:0.2f];
}

- (void)doneButtonClick {
//    md(@"", @"raindrop-add-content-done-c1", @"");
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                //NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:LANGUAGE(@"Select a maximum of %zd photos")], tzImagePickerVc.maxImagesCount];
                NSString *title = [NSString stringWithFormat:@"%@%ld%@", LANGUAGE(@"brandu_selectphoto_max1"),  (long)tzImagePickerVc.maxImagesCount, (tzImagePickerVc.maxImagesCount > 1 ? LANGUAGE(@"brandu_selectphoto_max2") : LANGUAGE(@"brandu_selectphoto_max4"))];
                [tzImagePickerVc showAlertWithTitle:title];
            }
        }
        
        [[NSUserDefaults standardUserDefaults] setInteger:indexPath.item forKey:@"SelectedImageIndexKey"];
...
//    [[NSUserDefaults standardUserDefaults] synchronize];
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        [[NSUserDefaults standardUserDefaults] setInteger:indexPath.item forKey:@"SelectedVideoIndexKey"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            int duration = phasset.duration ;
            if (duration < 3) {
                TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
                [imagePickerVc showAlertWithTitle:LANGUAGE(@"raindrop_video_edit_time_min")];
                return;
            }
            else if (duration > 600) {
                TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
                [imagePickerVc showAlertWithTitle:LANGUAGE(@"raindrop_video_edit_time_max")];
                return ;
            }
            ZLPhotoModel *zlModel = [ZLPhotoModel modelWithAsset:model.asset type:ZLAssetMediaTypeVideo duration:@"300"];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied) && iOS7Later) {
        ...
        if (iOS8Later) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle tz_localizedStringForKey:@"brandu_no_camera"] message:message delegate:self cancelButtonTitle:[NSBundle tz_localizedStringForKey:@"Cancel"] otherButtonTitles:[NSBundle tz_localizedStringForKey:@"Setting"], nil];
            [alert show];
        } else {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle tz_localizedStringForKey:@"brandu_no_camera"] message:message delegate:self cancelButtonTitle:[NSBundle tz_localizedStringForKey:@"OK"] otherButtonTitles:nil];
            [alert show];
        }
...
- (void)scrollCollectionViewToBottom {
    ...
    if (_shouldScrollToBottom && _models.count > 0) {
        ...
        TZAssetModel *tempModel = _models[0];
        NSInteger selectedItem = 0;
        if (tempModel.type == TZAssetModelMediaTypeVideo) {
            selectedItem = [[NSUserDefaults standardUserDefaults] integerForKey:@"SelectedVideoIndexKey"];
        } else {
            selectedItem = [[NSUserDefaults standardUserDefaults] integerForKey:@"SelectedImageIndexKey"];
        }
//        NSInteger selectedItem = [[NSUserDefaults standardUserDefaults] integerForKey:@"SelectedImageIndexKey"];
        if (selectedItem >= 20 && _models.count >= 20 && selectedItem < _models.count) {
            item = selectedItem;
            position = UICollectionViewScrollPositionCenteredVertically;
            _markButton.alpha = 1.0;
        }
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self->_collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:item inSection:0] atScrollPosition:position animated:NO];
...
//                ZLPhotoModel *zlModel = [ZLPhotoModel modelWithAsset:assetModel.asset type:ZLAssetMediaTypeVideo duration:@"300"];
//                ZLEditVideoController *vc = [[ZLEditVideoController alloc] init];
//                vc.model = zlModel;
//                [self.navigationController pushViewController:vc animated:NO];
